...
var app = (function () {
    ...
    function Legend(color, {
      ...
      if (color.interpolate) {
        ...
        x = color.copy().rangeRound(quantize$2(interpolate$2(marginLeft, width - marginRight), n));
        ...
        svg.append("image")
            ...
            .attr("xlink:href", ramp(color.copy().domain(quantize$2(interpolate$2(0, 1), n))).toDataURL());
      }
      ...
      // Sequential
        ...
        if (!x.ticks) {
          if (tickValues === undefined) {
            ...
            tickValues = range$2(n).map(i => quantile$1(color.domain(), i / (n - 1)));
          }
      ...
      // Threshold
        ...
        tickValues = range$2(thresholds.length);
    ...
    function create_fragment(ctx) {
    	...
    	const block = {
    		c: function create() {
    			...
    			div1 = element("div");
    			h1 = element("h1");
    			h1.textContent = "How does median income vary across the US?";
    			t1 = space();
    			div0 = element("div");
    			t2 = space();
    			svg = svg_element("svg");
    			add_location(h1, file, 188, 4, 5469);
    			attr_dev(div0, "class", "legend");
    			attr_dev(div0, "id", "legend-container");
    			add_location(div0, file, 189, 4, 5526);
    			add_location(svg, file, 190, 4, 5575);
    			attr_dev(div1, "class", "main");
    			add_location(div1, file, 187, 2, 5445);
    			add_location(main, file, 186, 0, 5435);
    		},
    ...
    function instance($$self, $$props, $$invalidate) {
    	...
    	onMount(async () => {
    		const us = await json("/states-albers-10m.json");
    		const csvData = await csv("/census_cleaned.csv");
    	...
    	function initializeChart(us) {
    		...
    		const colorScale = linear().domain(domain).range(["lightgreen", "darkgreen"]); //d3.extent(Array.from(incomeData.values()))
    		// Updated color scale from light green to dark green

    		const zoom$1 = zoom().scaleExtent([1, 8]).on("zoom", zoomed);
    		const svg = select(svgElement).attr("viewBox", `0 0 ${width} ${height}`).attr("preserveAspectRatio", "xMidYMid meet").attr("width", width).attr("height", height).style("display", "block").style("margin", "auto").style("overflow", "hidden").on('click', reset);
    		const path = index$2();
    		const g = svg.append("g");

    		let states = g.selectAll("path").data(feature(us, us.objects.states).features).join("path").on('click', clicked).attr('d', path).attr("fill", d => {
    			const income = incomeData.get(d.properties.name);
    			return income ? colorScale(income) : "#cccccc";
    		}).attr("d", path).append("title").text(d => `${d.properties.name}: $${Number(incomeData.get(d.properties.name)).toLocaleString() || 'No data'}`).exit();

    		// const labels = g.selectAll('.label')
    		//   .data(topojson.feature(us, us.objects.states).features)
    		//     .enter()
    		//   ;
    		g.append("path").attr("fill", "none").attr("stroke", "white").attr("stroke-linejoin", "round").attr("d", path(mesh(us, us.objects.states, (a, b) => a !== b)));

    		states = states.enter().append('text').attr("class", "label").attr("transform", d => "translate(" + path.centroid(d) + ")").text(d => `${stateCodes[d.properties.name]}\n $${Number(incomeData.get(d.properties.name)).toLocaleString()}`);
    		g.selectAll('.label').data(feature(us, us.objects.states).features).enter();
    		svg.call(zoom$1);
    		...
    		function clicked(event, d) {
    			...
    			console.log(data);
    			svg.transition().duration(750).call(zoom$1.transform, identity$1.translate(width / 2, height / 2).scale(Math.min(8, 0.9 / Math.max((x1 - x0) / width, (y1 - y0) / height))).translate(-(x0 + x1) / 2, -(y0 + y1) / 2), pointer(event, svg.node()));
    		}

    		function reset() {
    			console.log('hi');
    			svg.transition().duration(750).call(zoom$1.transform, identity$1, transform$1(svg.node()).invert([width / 2, height / 2]));
    		}
    	...
    	function createLegend() {
    		const legendContainer = select('#legend-container');

    		const legend = Legend(linear(domain, ['lightgreen', 'darkgreen']), {
    	...
    	$$self.$inject_state = $$props => {
    		if ('svgElement' in $$props) $$invalidate(0, svgElement = $$props.svgElement);
    		if ('legend' in $$props) legend = $$props.legend;
