...
var app = (function () {
    ...
    function create_fragment(ctx) {
    	...
    	const block = {
    		c: function create() {
    			...
    			t2 = space();
    			svg = svg_element("svg");
    			add_location(h1, file, 72, 4, 2319);
    			attr_dev(div0, "class", "legend");
    			attr_dev(div0, "id", "legend-container");
    			add_location(div0, file, 73, 4, 2376);
    			add_location(svg, file, 74, 4, 2425);
    			attr_dev(div1, "class", "main");
    			add_location(div1, file, 71, 2, 2295);
    			add_location(main, file, 70, 0, 2285);
    		},
    ...
    function instance($$self, $$props, $$invalidate) {
    	...
    	function initializeChart(us) {
    		...
    		const svg = select(svgElement).attr("viewBox", `0 0 ${width} ${height}`).attr("preserveAspectRatio", "xMidYMid meet").attr("width", width).attr("height", height).style("display", "block").style("margin", "auto").style("overflow", "visible");
    		const path = index$2();
    		const g = svg.append("g");

    		g.selectAll("path").data(feature(us, us.objects.states).features).join("path").attr("fill", d => {
    			const income = incomeData.get(d.properties.name);
    			return income ? colorScale(income) : "#cccccc";
    		}).attr("d", path).append("title").text(d => `${d.properties.name}: $${incomeData.get(d.properties.name) || 'No data'}`);
    	...
    	function createLegend() {
    		...
    		const legend = Legend(linear(domain, ['lightgreen', 'darkgreen']), { title: 'Median Income', ticks: 5 });
